...
sub SEQCOMPARE {
	...
	print FILE "LOD\tDistance (kb)\tMarker A Sites\tMarker B Sites\tTheta\tJones LOD\n";
	close FILE;
	...
	foreach $f (@fileList) {
		...
		for $col (0..($#group - 1)) {
			...
			for $row (($col+1)..$#group) {
				...
				printf FILE ("%s\t%.3f\t%s\t%s\t%.3f\t%s\n", int(0.5 + 1000*$theLOD)/1000, $dist/1000, $fullSites{$aMarker}, 
						$fullSites{$bMarker}, $theta, $jonesLOD);
			}
		}
		close FILE;
		print "</PRE>";
...
sub PRINTOPTIONS {
	...
	print qq(<FONT COLOR=orange><A HREF="SimulateRH.cgi?seqCompare=Y&dum=$$">Compare to Sequence</A></FONT><BR>\n);
...
sub calcProbs {
	...
	my $outputRef	= shift;								# [1] Ref to hash that will hold the final variables
	%{$outputRef}	= ();
