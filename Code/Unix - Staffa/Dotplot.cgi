...
print "<html><TITLE>Mega Dot Plot</TITLE><BODY BGCOLOR=White>";
&PRINTOPTIONS;

if ($filename) {
	my @Sequences = my @Names = ();
	my $numLines = &LOADFASTA($filename, \@Sequences, \@Names);

	$xName = $Names[0];
	$primarySeq  = $Sequences[0];
	$xLength = length ($primarySeq);
	my $num = $#Names + 1;
	if ($num > 1) {
		print "<FONT COLOR=RED><B>NOTE:</B></FONT>The primary file contained $num sequences. The first sequence has been chosen as the X-axis sequence.<BR>\n";
	}
	print "<FONT COLOR=brown><B>Primary file <FONT COLOR=blue>$filename</FONT> contains $numLines lines and $xLength bp.</B></FONT><BR>\n";
	$data = "";
	$xSeq = \$primarySeq;
	if ($othername) {
		@Sequences = my @Names = ();
		$numLines = &LOADFASTA($othername, \@Sequences, \@Names);
		$yName = $Names[0];
		$secondarySeq  = $Sequences[0];
		$yLength = length ($secondarySeq);
		$num = $#Names + 1;
		if ($num > 1) {
			print "<FONT COLOR=RED><B>NOTE:</B></FONT>The secondary file contained $num sequences. The first sequence has been chosen as the X-axis sequence.<BR>\n";
		}
		print "<FONT COLOR=brown><B>Secondary file <FONT COLOR=blue>$othername</FONT> contains $numLines lines and $yLength bp.</B></FONT><BR>\n";
		$data = "";
		$ySeq = \$secondarySeq;
	} else {
		$ySeq = $xSeq;
	...
	$DotTime = time;
	my $gph = &MEGADOT($xSeq, $ySeq, $xName, $yName, $dotWord, $pixSize);
	...
	$SquareKb = $xLength * $yLength / 1000000;								# The Area of the plot, in square kilobases
	if ($DotTime == 0) {
		$rate = "many, many";
	} else {
		$rate = int (0.5 + $SquareKb / $DotTime);
	}
	print "<i>$DotTime seconds to perform dot plot = $rate square kb per second.</i><BR>\n";
}
